...
resnet_conv = ResNet50(weights=None, include_top=False, input_shape=(image_size, image_size, 3))
...
filepath = "current_best_weights_resnet50_machine_label_balance.hdf5"
...
EPOCHS = 50
BATCH_SIZE = 256
history1 = model.fit(X_train, Y_train, batch_size = BATCH_SIZE, epochs = EPOCHS, validation_data = (X_val, Y_val),  callbacks = [early_stopping, checkpoint], verbose = 1)
...
writer = csv.writer(open('results/Resnet50_train_with_generated_label_none.csv','w'))
...
model.save('trained_models/Resnet50_train_with_generated_label_none.h5')
